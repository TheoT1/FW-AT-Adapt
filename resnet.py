...
class ResNetNI_layer3(nn.Module):
    """ ResNet class with Noise Injection at intermediate layer 3 """
    # feat_scale lets us deal with CelebA, other non-32x32 datasets
    def __init__(self, block, num_blocks, num_classes=10, feat_scale=1, wm=1):
        super(ResNetNI_layer3, self).__init__()
    ...
    def forward(self, x, noise, with_intermediate=False, with_latent=False, fake_relu=False, no_relu=False):
        assert (not no_relu),  \
            "no_relu not yet supported for this architecture"
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        ...
        if with_intermediate:
            return final, z
        elif with_intermediate and with_latent:
            return final, z, pre_out
        if with_latent:
            return final, pre_out
        return final

class ResNetNI_layer4(nn.Module):
    """ ResNet class with Noise Injection at intermediate layer 4 """
    # feat_scale lets us deal with CelebA, other non-32x32 datasets
    def __init__(self, block, num_blocks, num_classes=10, feat_scale=1, wm=1):
        super(ResNetNI_layer4, self).__init__()
...
def ResNet18_layer3(**kwargs):
    return ResNetNI_layer3(BasicBlock, [2,2,2,2], **kwargs)

def ResNet18_layer4(**kwargs):
    return ResNetNI_layer4(BasicBlock, [2,2,2,2], **kwargs)
...
resnet18_lay3 = ResNet18_layer3
resnet18_lay4 = ResNet18_layer4
resnet18 = ResNet18
resnet50 = ResNet50
resnet101 = ResNet101
resnet152 = ResNet152
